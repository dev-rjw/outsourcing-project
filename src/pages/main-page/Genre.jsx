...
const GenreButton = ({ children, idx, clicked, setClicked }) => {
  ...
  if (clicked === idx) {
    return (
      <button
        className="h-fit w-fit px-4 py-2 mr-5 last:mr-0 bg-primary text-white border border-solid border-primary rounded-3xl cursor-pointer"
        onClick={() => {
          handleGenreClick(idx);
        }}
        key={idx}
      >
        {children}
    ...
    return (
      <button
        className="h-fit w-fit px-4 py-2 mr-5 last:mr-0 bg-white border border-solid border-gray-300 rounded-3xl cursor-pointer"
        onClick={() => {
          handleGenreClick(idx);
        }}
        key={idx}
